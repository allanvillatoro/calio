...
import { resolveProductDetailItem } from './product-detail.helpers';
...
describe('resolveProductDetailItem', () => {
  ...
  it('tries slug lookups after a numeric id misses', async () => {
    vi.mocked(laserEngravingsRepository.findBySlug).mockResolvedValue(
      laserEngraving,
    );

    await expect(resolveProductDetailItem('99')).resolves.toEqual({
      kind: 'laser-engraving',
      item: laserEngraving,
    });
    expect(productsRepository.findById).toHaveBeenCalledWith(99);
    expect(productsRepository.findBySlug).toHaveBeenCalledWith('99');
    expect(laserEngravingsRepository.findBySlug).toHaveBeenCalledWith('99');
  });
});
